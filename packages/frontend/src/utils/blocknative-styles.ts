...
export default css`
  .bn-onboard-custom {
    ...
    color: ${({ theme }): string => theme.text.primary} !important;
    font-family: ${({ theme }): string => theme.font.standard} !important;
    z-index: ${Z_INDEX.onboardModal};
    .bn-onboard-modal-content {
      border-radius: ${({ theme }): string => `${theme.borderRadius * 2}px`};
      background-color: ${({ theme }): string => theme.bg.primary};
    }
    ...
    .bn-onboard-prepare-button:hover {
      background-color: ${({ theme }): string => theme.bg.primary} !important;
    }
